...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    dists = None
    # ====== YOUR CODE: ======
    dists = ((x1**2).sum(1).reshape(-1, 1) + (x2**2).sum(1) - (2 * (x1.matmul(x2.T)))).sqrt()
    # ========================
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    accuracy = None
    # ====== YOUR CODE: ======
    accuracy = (y.shape[0] - torch.count_nonzero(y - y_pred).item()) / y.shape[0]
    # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for j in range(num_folds):
            dl_train, dl_valid = hw1dataloaders.create_train_validation_loaders(ds_train, 1 / num_folds)
            model.train(dl_train=dl_train)

            x_test, y_test = dataloader_utils.flatten(dl_valid)
            y_pred = model.predict(x_test)
            k_choise_accs.append(accuracy(y_test, y_pred))
        accuracies.append(k_choise_accs)
        # ========================
