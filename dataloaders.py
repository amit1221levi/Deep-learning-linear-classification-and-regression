...
class FirstLastSampler(Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        for i in range(len(self.data_source)):
            yield  (i // 2)  if (i%2 == 0) else (len(self.data_source) - (i // 2) - 1)
        # ========================
...
def create_train_validation_loaders(
    ...
    #  you create.
    # ====== YOUR CODE: ======
    split_idx = round((1 - validation_ratio) * len(dataset))

    train_sampler = torch.utils.data.SubsetRandomSampler(range(split_idx))
    valid_sampler = torch.utils.data.SubsetRandomSampler(range(split_idx, len(dataset)))
